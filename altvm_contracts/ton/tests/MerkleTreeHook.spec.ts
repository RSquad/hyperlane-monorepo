...
describe('MerkleTreeHook', () => {
    ...
    beforeEach(async () => {
        ...
        const deployResult = await merkleTreeHook.sendDeploy(deployer.getSender(), toNano('0.05'));
    ...
    it('should post dispatch', async () => {
        const res = await merkleTreeHook.sendPostDispatch(deployer.getSender(), toNano('0.1'), {
            messageId: 1n,
            destDomain: 0,
